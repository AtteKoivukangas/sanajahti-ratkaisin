function solverWorker() {
  const findStringsThatCanBeFormedFromAnotherString = (
    ...
    return stringArray.filter((str) => {
      ...
      for (let i = str.length - 1; i >= 0; i--)
        strCharCounts[str[i].toUpperCase().charCodeAt(0)]++;
      for (let i = 65; i < 91; i++) return charCounts[i] >= strCharCounts[i]; // A-Z
      return (
        charCounts[196] >= strCharCounts[196] ||
        charCounts[214] >= strCharCounts[214]
      ); // Ä&Ö
    });
  };
  ...
  const wordzDFS = (boardData, words) => {
    ...
    const filteredWords = findStringsThatCanBeFormedFromAnotherString(
      words,
      boardData
      ...
      .forEach((wordToFind) => {
        ...
        const dfs = (y, x) => {
          if (
            ...
            y > 3 ||
            x < 0 ||
            x > 3 || // outside of board?
            visitedSlots[y][x] ||
            isWordBuiltSuccesfully || // slot already visited or word already found?
